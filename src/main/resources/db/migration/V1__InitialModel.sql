...
create table if not exists tasks (
  ...
  constraint room_id_fk             foreign key (room_id) references rooms(id) on delete cascade,
  ...
  constraint default_priority_limit check (default_priority > 0),
  constraint current_priority_limit check (current_priority >= default_priority),
...
insert into goals(goal, score_cap) values ('Общая копилка', 2000000000);